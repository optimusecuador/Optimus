...
.menu-section a:hover{transform:translateX(6px);background:rgba(0,255,255,.08)}
    ...
    <main class="content">
      ...
      <!-- InstanceBeginEditable name="principal" -->
						<?php
// Configuración de fechas para el último mes por defecto
$fechaActual = date('Y-m-d');
$fechaHaceUnMes = date('Y-m-d', strtotime('-30 days'));

// Captura de datos
$institucion = $_POST['institucion'] ?? "";
$fecha1 = $_POST['fecha'] ?? $fechaHaceUnMes;
$fecha2 = $_POST['fecha2'] ?? $fechaActual;

// Lógica de carga inicial: si no hay busqueda, configuramos los valores del último mes
$es_busqueda = isset($_POST['guardar']);
if (!$es_busqueda) {
    // Seleccionar la primera cuenta disponible por defecto para el reporte inicial
    $resInicial = mysqli_query($con, "SELECT numero FROM cuentas ORDER BY numero ASC LIMIT 1");
    if($rowI = mysqli_fetch_assoc($resInicial)) { $institucion = $rowI['numero']; }
}

$resultCuentas = mysqli_query($con, "SELECT * FROM cuentas ORDER BY numero ASC");
?>

<div class="panel-dark">
    <table width="100%" align="center" class="table-dark">
        <tr><td align="center"><h2>INGRESOS BANCOS</h2></td></tr>
        <tr>
            <td align="center">
                <form action="" method="post" name="form1">
                    ...
                    <select name="institucion" class="clientes-input">
                        <?php 
                        mysqli_data_seek($resultCuentas, 0);
                        while($row = mysqli_fetch_assoc($resultCuentas)) echo "<option value='{$row['numero']}' ".(($institucion == $row['numero'])?'selected':'').">{$row['institucion']}</option>"; 
                        ?>
                    </select></p>
                    <p>Desde: <input type="date" name="fecha" value="<?php echo $fecha1; ?>" > 
                       Hasta: <input type="date" name="fecha2" value="<?php echo $fecha2; ?>" ></p>
                    <button type="submit" name="guardar" class="boton-azul">Buscar</button>
                </form>
                
                <div style="margin-top:15px;">
                    <button class="boton-azul" onclick="exportarExcel('tablaResultados')">Exportar a Excel</button>
                    <button class="boton-azul" onclick="window.print()">Imprimir Tabla</button>
                </div>
            </td>
        </tr>
        <tr>
            <td>
                <table width="95%" align="center" class="clase_tabla" id="tablaResultados">
                    <thead>
                        <tr>
                            <td><strong>FECHA</strong></td><td><strong>FACTURA</strong></td><td><strong>RECIBO</strong></td>
                            <td><strong>CONCEPTO</strong></td><td><strong>IMAGEN</strong></td><td><strong>CLIENTE</strong></td>
                            <td><strong>USUARIO</strong></td><td><strong>ACCION</strong></td><td><strong>VALOR</strong></td><td><strong>SALDO</strong></td>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        if ($institucion != "") {
                            $query = "SELECT * FROM registro WHERE proveedor LIKE '$institucion' AND fecha BETWEEN '$fecha1' AND '$fecha2' AND numerorecibo != 'Sin_Recibo' ORDER BY fecha ASC";
                            $res = mysqli_query($con, $query);
                            $saldo2 = 0;

                            while ($row = mysqli_fetch_assoc($res)) {
                                $rVenta = mysqli_query($con, "SELECT recibo, nombrecliente FROM ventas WHERE numerorecibo = '{$row['numerorecibo']}'");
                                $dataV = mysqli_fetch_assoc($rVenta);
                                $valor = $row['saldo'];
                                if ($row['accion'] == "ingreso" || $row['accion'] == "pago") $saldo2 += $valor;
                                else $saldo2 -= $valor;
                        ?>
                        <tr>
                            ...
                            <td>
                                <?php if (!empty($dataV['recibo']) && $dataV['recibo'] != "../images/sin_documento.png"): ?>
                                    <img src="<?php echo $dataV['recibo']; ?>" width="50">
                                <?php endif; ?>
                            </td>
                            <td><?php echo $dataV['nombrecliente'] ?? ''; ?></td>
...
function exportarExcel(tablaID) {
    ...
    // Creamos un blob con el contenido de la tabla
    var url = 'data:application/vnd.ms-excel,' + escape(html);
    var link = document.createElement("a");
    link.download = "Reporte_Bancos.xls";
    link.href = url;
    link.click();
}
</script>
...
<style type="text/css" media="print">
    /* Estilos para que al imprimir solo se vea la tabla */
    body * { visibility: hidden; }
    #tablaResultados, #tablaResultados * { visibility: visible; }
    #tablaResultados { position: absolute; left: 0; top: 0; width: 100%; }
</style>
