    ...
    <div class="clientes-header-top">
        ...
        <div>
            ...
            <h2 class="clientes-title">
                CREAR CONTRATOS Y CARPETAS
            </h2>

            <p class="clientes-subtitle">
                Genera contratos automáticamente para los clientes importados y crea sus carpetas correspondientes.
            </p>
...
$resNodo = $conn->query("SELECT codigo, puesto FROM nodo");

while ($rowNodo = $resNodo->fetch_assoc()) {

    $nodos[] = $rowNodo;
}

// ============================
// OBTENER PRODUCTOS MENSUALES
// ============================
$productos = [];
...
if (isset($_POST['importar'])) {
    ...
    $res = $conn->query("SELECT * FROM migrar");
    ...
    while ($row = $res->fetch_assoc()) {
        ...
        $producto_codigo = $_POST['producto'][$codigo_cliente] ?? "SIN_PRODUCTO";

        $terceraedad = $_POST['terceraedad'][$codigo_cliente] ?? "no";

        $nodo = $_POST['nodo'][$codigo_cliente] ?? "SIN_NODO";
    ...
    while($crowa = mysqli_fetch_assoc($codigocomp)) {
        ...
        $estructura = '../clientes/fotos/'.$codigo;
    ...
    $sqlcom = "SELECT * from clientes order by nombres DESC";
    ...
    while($crowa = mysqli_fetch_assoc($codigocomp)) {
        ...
        $nombre = $crowa['nombres'];

        $direccion = $crowa['direccion'];
        ...
        if($codigo != $codigocomparar) {
            ...
            $codigon = mysqli_query($con, $sqln);

            while($crown = mysqli_fetch_assoc($codigon)) {
            ...
            $resultado = mysqli_query($con, $sqlg);
        }
    }
    ...
    if ($contador > 0) {
        ...
        $conn->query("TRUNCATE TABLE migrar");

?>

<script>

alert("✅ Importados: <?= $contador ?> registros\nImportacion Creada Satisfactoriamente\nCreados: <?= $contador ?> registros");
...
alert("❌ No se importaron registros");
            ...
            <table class="table-dark">
                ...
                <thead>
                    ...
                    <tr>
                        ...
                        <th align="center">Plan (Original)</th>

                        <th align="center">Productos Mensuales</th>

                        <th align="center">Tercera Edad</th>

                        <th align="center">Nodo</th>
                ...
                <tr>
                    ...
                    <td align="center">
                        ...
                        <select
                            class="clientes-input"
                            name="nodo[<?= $row['codigo'] ?>]"
                        >

                            <?php foreach ($nodos as $nod): ?>

                                <option value="<?= $nod['codigo'] ?>">
