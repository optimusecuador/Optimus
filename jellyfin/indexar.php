...
/* --- PARÁMETROS DE REPRODUCCIÓN (720p y 1.5 Mbps) --- */
$streamingParams = "VideoCodec=h264&AudioCodec=aac&MaxWidth=1280&MaxHeight=720&VideoBitRate=1500000&AudioBitRate=1500000&maxStreamingBitrate=1500000";
...
function getAudioTracksJson($movie, $apiKey, $serverBaseUrl, $streamingParams) {
    $streams = extractStreams($movie);
    $tracks = [];
    foreach ($streams as $s) {
        if (($s['Type'] ?? '') === 'Audio') {
            ...
            $audioUrl = $serverBaseUrl . "/Videos/" . ($movie['Id'] ?? '') . "/stream?AudioStreamIndex=" . $index . "&api_key=" . $apiKey . "&" . $streamingParams;
...
if (!empty($allMoviesList)) {
    ...
    if ($stmt = $conexion->prepare($sqlSync)) {
        foreach ($allMoviesList as $m) {
            ...
            // --- GESTIÓN DE URLs (INTERNA Y PÚBLICA CON LÍMITE DE 720P Y 1.5MBPS) ---
            $peliculaUrl = $server_local . "/Videos/" . $idPel . "/stream?api_key=" . $apikey . "&" . $streamingParams;
            $peliculaUrlPublico = $server_publico . "/Videos/" . $idPel . "/stream?api_key=" . $apikey . "&" . $streamingParams;
            
            // --- GESTIÓN DE CAMPOS DE AUDIO ---
            $audioLangs = getLanguages($m);                              
            $peliculaAudio = getAudioTracksJson($m, $apikey, $server_local, $streamingParams); 
    ...
    <table>
        ...
        <tbody>
            ...
            foreach ($previewItems as $m) {
                $idPel = $m['Id'] ?? '';
                $nombre = $m['Name'] ?? '';
                $urlInt = $server_local . "/Videos/" . $idPel . "/stream?api_key=" . $apikey . "&" . $streamingParams;
                $urlPub = $server_publico . "/Videos/" . $idPel . "/stream?api_key=" . $apikey . "&" . $streamingParams;
                echo '<tr>';
