...
.menu-section a:hover{transform:translateX(6px);background:rgba(0,255,255,.08)}
	...
	<div>
    <h2 style='color:white'>MIKROTIK</h2>
</div>
										  <?php 
			$ip = "Vacio";
			$corte = "Vacio";
          ...
          <!-- Column -->
         <?php 
							$sqlmin = "SELECT * from `nodo`";
							$resultmin = mysqli_query($con, $sqlmin);
							while($crowmin = mysqli_fetch_assoc($resultmin))
							{
								$nodo = $crowmin['codigo']; 
								$sqlmi = "SELECT * from `mikrotik` WHERE `nodo` LIKE '$nodo'";
								$resultmi = mysqli_query($con, $sqlmi);
								while($crowmi = mysqli_fetch_assoc($resultmi))
								{
								
								$ip = $crowmi['ip'];
								$corte = $crowmi['corte'];
								$backup = $crowmi['backup'];
								$creacion = $crowmi['creacion'];
								$usuario = $crowmi['usuario'];
								$reactivacion = $crowmi['reactivacion'];
								$reinicio = $crowmi['reinicio'];
								$eliminacion = $crowmi['eliminacion'];
								$contrasena = $crowmi['contrasena'];
							}
							?>	
							<div class="panel-dark">
								
								<div class="cliente-info-title">
                				<?php echo $nodo = $crowmin['codigo'];?>
            					</div>
								
							  <form action="guardar_mikrotik.php" method="post" name="form1" id="form1">
							    <table width="90%" class="table-dark">
							      <tbody>
							        <tr>
							          <td><strong>Ip de acceso:</strong></td>
							          <td><input class="clientes-input" name="ip" type="text" id="ip" value="<?php echo $ip; ?>"></td>
							          <td><strong>Usuario:</strong></td>
							          <td><input class="clientes-input" name="usuario" type="text" id="usuario" value="<?php echo $usuario; ?>"></td>
							          <td>&nbsp;</td>
						            </tr>
							        <tr>
							          <td>&nbsp;</td>
							          ...
							          <td><input class="clientes-input" name="contrasena" type="password" id="contrasena" value="<?php echo $contrasena; ?>"></td>
							          <td>&nbsp;</td>
						            </tr>
							        <tr>
							          <td><strong>Cortes Automaticos</strong></td>
							          <td><input class="clientes-input" name="corte" type="text" id="corte" value="<?php echo $corte; ?>"></td>
							          <td><strong>Comando de reactivacion:</strong></td>
							          <td><input class="clientes-input" name="reactivacion" type="text" id="reactivacion" value="<?php echo $reactivacion; ?>"></td>
							          <td>&nbsp;</td>
						            </tr>
							        <tr>
							          <td><strong>Comando de Backup:</strong></td>
							          <td><input class="clientes-input" name="backup" type="text" id="backup" value="<?php echo $backup; ?>"></td>
							          <td><strong>Comando de reinicio:</strong></td>
							          <td><input class="clientes-input" name="reinicio" type="text" id="reinicio" value="<?php echo $reinicio; ?>"></td>
							          <td>&nbsp;</td>
						            </tr>
							        <tr>
							          <td><strong>Comando de Creacion:</strong></td>
							          <td><input class="clientes-input" name="creacion" type="text" id="creacion" value="<?php echo $creacion; ?>"></td>
							          <td><strong>Comando Eliminacion:</strong></td>
							          <td><input class="clientes-input" name="eliminacion" type="text" id="eliminacion" value="<?php echo $eliminacion; ?>"></td>
							          <td>&nbsp;</td>
						            </tr>
							        <tr>
							          <td colspan="5" align="center"><span class="buttons">
							            <?php 
									  if ($ip == "Vacio")
									  {
										  $accion = "nuevo";
									  ...
									  ?>
							            ...
							            <input name="iporiginal" type="hidden" id="iporiginal" value="<?php echo $ip; ?>">
