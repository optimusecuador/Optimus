...
<div>
<div>
    <h2 style='color:white'>ACTIVACION DE CLIENTES <?php echo $tipoactivacion?></h2>
</div>
        ...
        <table width="100%"  align="center">
          <tbody>
			...
			if (isset($_GET['codigo']) OR isset($_POST['contrato_manual'])) 
                        {
				...
				if (isset($_POST['contrato_manual'])) 
                {
					...
					$serie_manual=$_POST['serie_manual'];
				}
				else
				{
			...
			?>
				...
				// EJECUTAR COMANDOS DE ACTIVACION DE OLT 
					...
					if ($posicion_coincidencia === false) 
					{
    					if (isset($_GET['codigo'])) 
                	{
						$serieont = $_GET['serieont'];
						$tarjeta = $_GET['tarjeta'];
							$output;
                        // saco ytarjeta y puerto
                        $separador = "F/S/P";
                        $separada = explode($separador, $output);
						$cantidadDeclientes-input-small = count($separada);	
                        for ($onus = 1; $onus < $cantidadDeclientes-input-small; $onus++) 
                        {
							?>
							  <?php 
								//echo $separada[$onus];
								$frase = $separada[$onus];
								if (str_contains($frase, $serieont)) 
								{
    							//echo "esta ont es para activar"; echo $serieont;
								}
							?>
                            <br>
                            
							<span class="buttons">
                            		<input name="numero" type="hidden" id="numero" form="form1" value=<?php echo $codigo_contrato;?>>
							  		<input name="codigo" type="hidden" id="codigo" value="<?php echo $codigo_contrato;?>">
							</span>
                          <form action="servicio_tecnico_oficina_activacion_dos.php" method="post" name="form1" id="form1">
				...
				// EJECUTAR COMANDOS DE ACTIVACION DE OLT 
					...
					if ($posicion_coincidencia === false) 
					{
    					if (isset($_GET['codigo'])) 
                	{
						$puertoytarjeta = $_GET['tarjeta'];
						
					}
						else
					{
							$puertoytarjeta = " ".$puerto_manual;
							$puertoytarjetapuestooriginal = $puerto_manual;
					}
                        ?>
						<div>
    					<h2 style='color:white'>INFORMACION DE ONT</h2>
						</div>	  
						<?php 
						echo "TARJETA Y PUERTO : ";echo $puertoytarjeta = $tarjeta;	
                        $puertoytarjetapuestooriginal  = $puertoytarjeta;
						...
						$puertoytarjetapuestooriginal;
							  	?>
                                        ...
                                        echo "SERIE DE LA ONT : ";echo $serieont;
					...
					?>
                                        <div>
										<h2 style='color:white'>EQUIPOS ACTUALES</h2>
										</div>	
                                        <?php
										...
										?>
							  			<div>
										<h2 style='color:white'>PRECOMANDOS DE ACTIVACION</h2>
										</div>	
...
//                                        }
					  ...
					  <?php
						...
						//FIN DE MOSTRAR NODO PARA ACTIVACION
							 			?>
										<div>
    									<h2 style='color:white'>COMANDOS OLT</h2>
										</div>
							  			<?php
							  			?>
                                      ...
                                      ?>
           							...
           							?>
    									...
    									<h2 style='color:white'>MONITOREO</h2>
							  			...
							  			<?php
                                        echo $commandocho = '/tool netwatch add host='.$ip_mikrotik.' timeout=1s interval=00:01:00 up-script="/log error \"'.$nombre_contrato.' El host está UP\"" down-script="/log error \"'.$nombre_contrato.' El host está DOWN\""'."\n";
                                        //echo $commandocho = 'ip firewall address-list add list="clientes_cuenca" address='.$ip_mikrotik.' comment="'.$nombre_contrato.'"'."\n";
						...
						$puerto_gestion = "9090";
						$sqlip = "UPDATE contratos SET ip='$ip_mikrotik', puerto_gestion='$puerto_gestion', router='$separadaserie[0]' WHERE numero='$codigo_contrato'";
                          ...
                          <td align="center" valign="middle">
							  ...
							  if (isset($_POST['contrato_manual'])) 
								...
								?>
                            		...
                            		<input name="numero" type="hidden" id="numero" form="form1" value=<?php echo $codigo_contrato;?>>
							  		<input name="codigo" type="hidden" id="codigo" value="<?php echo $codigo_contrato;?>">
									<input name="tarjeta" type="hidden" id="tarjeta" value="<?php echo $separadaf[0]."/".$separadaf[1];?>">
									<input name="service_port" type="hidden" id="service_port" value="<?php echo $serviceportfinal;?>">
									<input name="ip" type="hidden" id="ip" value="<?php echo $ip_mikrotik;?>">
									<input name="nombre_cliente" type="hidden" id="nombre_cliente" value="<?php echo $nombre_contrato;?>">
									<input name="subida" type="hidden" id="subida" value="<?php echo $subida;?>">
									<input name="bajada" type="hidden" id="bajada" value="<?php echo $bajada;?>">
									<input name="vlan" type="hidden" id="vlan" value="<?php echo $vlan_base;?>">
									<input name="interface" type="hidden" id="interface" value="<?php echo $codigo_contrato;?>">
									<input name="tarjeta" type="hidden" id="tarjeta" value="<?php echo $separadafdos;?>">
									<input name="posicion" type="hidden" id="posicion" value="<?php echo $imprimirfinal;?>">
									<input name="serial" type="hidden" id="serial" value="<?php echo $separadaserie[0];?>">
									<input name="nodo" type="hidden" id="nodo" value="<?php echo $nodo;?>">
									<input name="puertoytarjeta" type="hidden" id="nodo" value="<?php echo $puertoytarjeta;?>">
								...
								?>
                            		...
                            		<input name="numero" type="hidden" id="numero" form="form1" value=<?php echo $codigo_contrato;?>>
							  		<input name="codigo" type="hidden" id="codigo" value="<?php echo $codigo_contrato;?>">
                           			...
                           			<br>
                         			<input name="submit2" type="submit" class="boton-azul" id="submit2" value="ACTIVAR">
									<br>
                          			</span>
							  	<?php
								}
