...
.menu-section a:hover{transform:translateX(6px);background:rgba(0,255,255,.08)}
    ...
    <main class="content">
      ...
      <!-- InstanceBeginEditable name="principal" -->
									<div>
    <h2 class="titulo-form">CARGAR COMPROBANTE</h2>
</div>

<?php 
$tabla = "ventas";
$_SESSION['control'] ="si";
$_SESSION['form'] = "factura";

// Lógica de subida (mantenida intacta)
if (isset($_POST['subir'])) {
    ...
    if (isset($archivo) && $archivo != "") {
        ...
        if (!((strpos($tipo, "gif") || strpos($tipo, "jpeg") || strpos($tipo, "jpg") || strpos($tipo, "png")) && ($tamano < 200000000))) {
            echo '<div class="alerta-error"><b>Error. Formato o tamaño incorrecto.</b></div>';
        } else {
            $cliente = $_POST['cliente'];
            $id = $_POST['id'];
            
            if (move_uploaded_file($temp, '../clientes/fotos/'.$cliente.'/'.$archivo)) {
                chmod('../clientes/fotos/'.$cliente.'/'.$archivo, 0777);
                $foto = "../clientes/fotos/".$cliente."/".$archivo;
                
                $sql = "UPDATE ventas SET recibo='$foto' WHERE id='$id'";
                mysqli_query($con, $sql);
                mysqli_close($con);
                
                $_SESSION['foto'] = $foto;
                $contrato = $_SESSION['contratodos'];
                $_SESSION['ruta'] = "../clientes/fotos/".$cliente."/";
                ?>
                <script>
                    alert("✅ Foto subida satisfactoriamente");
                    window.location.href = "../notas/pago.php?codigo=<?php echo $id;?>&cliente=<?php echo $cliente;?>&contrato=<?php echo $contrato;?>";
                </script>
                <?php 
            } else {
                echo '<div class="alerta-error"><b>Ocurrió un error al subir el fichero.</b></div>';
            }
        }
    }
}
...
if (isset($_GET['codigo'])) {
    $codigo = $_GET['codigo'];
    $sql = "SELECT * from `".$tabla."` WHERE `id` LIKE '$codigo'";
    $result = mysqli_query($con, $sql); 
    while($crow = mysqli_fetch_assoc($result)) {	
        $codigo = $crow['id'];
        $producto = $crow['nombrecliente'];
...
<div class="panel">
    <table width="95%" align="center" class="tabla-dark">
        <tbody>
            <tr>
                <td align="center">
                    <h2 class="subtitulo"><?php echo $producto." ". $cliente; ?></h2>
                </td>
            </tr>
            <tr>
                <td align="center">
                    <form action="subir_foto.php" method="POST" enctype="multipart/form-data">
                        ...
                        <br>	
                        <input class="campos" name="archivo" id="archivo" type="file"/>
                        <br><br>
                        
                        <input class="boton-azul" type="submit" name="subir" value="Subir imagen"/>
                    </form>	
                </td>
            </tr>
        </tbody>
