...
.menu-section a:hover{transform:translateX(6px);background:rgba(0,255,255,.08)}
    ...
    <main class="content">
      ...
      <!-- InstanceBeginEditable name="principal" -->
							<?php
$tabla = "registro";
$sql = "SELECT * FROM `$tabla` WHERE `observacion` LIKE '%transferencia%' ORDER BY fecha DESC"; 
// Ejecución de la consulta intacta
...
<body>
    ...
    <div class="panel">
        <div class="esquinas_redondeadas" style="text-align: center; margin-bottom: 20px;">
            <h2 id="optimus" style="color: var(--text); margin: 0;">LISTADO DE TRANSFERENCIAS BODEGA</h2>
        </div>

        <div class="grilla_listado" style="overflow-x: auto;">
            <table class="clase_tabla" style="width: 100%; border-collapse: collapse; text-align: left;">
                <thead>
                    <tr>
                        <th style="width:120px; text-align: center;">FECHA</th>
                        <th style="width:80px; text-align: center;">ID</th>
                        <th>B. ORIGEN</th>
                        <th style="text-align: center;">B. DESTINO</th>
                        <th>PRODUCTO</th>
                        <th style="text-align: center; width:130px;">DESCRIPCION</th>
                ...
                <tbody>
                    ...
                    while($crow = mysqli_fetch_assoc($result)) { 
                        // Alternancia de estados / líneas según tu lógica PHP
                        if($filacolor == "0") {
                            $color_clase = "fila-par";
                            $filacolor = "1";
                        } else {
                            $color_clase = "fila-impar";
                            $filacolor = "0";
                        }
                    ?>
                        <tr class="alternar <?php echo $color_clase; ?>">
                            <td style="text-align: center;"><?php echo htmlspecialchars($crow['fecha']); ?></td>
                            <td style="text-align: center; font-weight: bold; color: var(--cyan);"><?php echo htmlspecialchars($crow['id']); ?></td>
                            <td><?php echo htmlspecialchars($crow['cliente']); ?></td>
                            <td style="text-align: center;"><?php echo htmlspecialchars($crow['bodega']); ?></td>
                            <td><?php echo htmlspecialchars($crow['producto']); ?></td>
                            <td style="text-align: center;"><?php echo htmlspecialchars($crow['observacion']); ?></td>
                            <td style="text-align: center;">
                                <a href="../bodegas/reimprimir_transferencia.php?codigo=<?php echo urlencode($crow['id']); ?>" class="btn-maps" style="display: inline-block; text-align: center;">
                                    Reimprimir
                                </a>
                            </td>
                        </tr>
