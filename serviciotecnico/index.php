...
/*=========================================
INDICADORES DE SERVICIO TÉCNICO
=========================================*/
// Total de servicios
$sql_total = "SELECT COUNT(*) total FROM clienteasignar";
$query_total = mysqli_query($con, $sql_total);
$total_servicios = mysqli_fetch_assoc($query_total)['total'];

// Pendientes (Ejemplo: donde estado sea 'Pendiente')
$sql_pendientes = "SELECT COUNT(*) total FROM clienteasignar WHERE estado = 'Pendiente'";
$total_pendientes = mysqli_fetch_assoc(mysqli_query($con, $sql_pendientes))['total'];

// En Proceso
$sql_proceso = "SELECT COUNT(*) total FROM clienteasignar WHERE estado = 'En Proceso'";
$total_proceso = mysqli_fetch_assoc(mysqli_query($con, $sql_proceso))['total'];

// Resueltos
$sql_resueltos = "SELECT COUNT(*) total FROM clienteasignar WHERE estado = 'Resuelto'";
$total_resueltos = mysqli_fetch_assoc(mysqli_query($con, $sql_resueltos))['total'];
...
$where = " WHERE 1=1 ";
if($buscar != '') {
    $where .= " AND (cliente LIKE '%$buscar%' OR codigo LIKE '%$buscar%' OR estado LIKE '%$buscar%')";
}

$sql_servicios = "SELECT * FROM clienteasignar $where ORDER BY fecha DESC";
$query_servicios = mysqli_query($con, $sql_servicios);
?>

<div class="cliente-wrapper">
    <div class="panel-dark">
        <h2 class="clientes-title">Gestión de Servicios Técnicos ISP</h2>
        
        <div class="cliente-grid" style="grid-template-columns: repeat(4, 1fr); margin-bottom: 20px;">
            <div class="panel-card" style="padding: 15px; background: var(--panel-2);">
                <div style="color: var(--muted);">Total Servicios</div>
                <div style="font-size: 24px;"><?php echo $total_servicios; ?></div>
            </div>
            <div class="panel-card" style="padding: 15px; background: #ff2f6530; color: var(--red);">
                <div>Pendientes</div>
                <div style="font-size: 24px;"><?php echo $total_pendientes; ?></div>
            </div>
            <div class="panel-card" style="padding: 15px; background: #ff8a1230; color: var(--orange);">
                <div>En Proceso</div>
                <div style="font-size: 24px;"><?php echo $total_proceso; ?></div>
            </div>
            <div class="panel-card" style="padding: 15px; background: #0faf7630; color: var(--green);">
                <div>Resueltos</div>
                <div style="font-size: 24px;"><?php echo $total_resueltos; ?></div>
            </div>
        </div>

        <form method="GET" style="margin-bottom: 20px;">
            <div style="display: flex; gap: 10px;">
                <input type="text" name="buscar" class="clientes-input" placeholder="Buscar cliente, código o estado..." value="<?php echo htmlspecialchars($buscar); ?>">
                <button type="submit" class="btn-action btn-edit">🔍 Buscar</button>
                <a href="index.php" class="btn-action">↻ Limpiar</a>
            </div>
        </form>
        ...
        <div class="table-scroll">
            <table class="table-dark">
                ...
                <tbody>
                    <?php while($row = mysqli_fetch_assoc($query_servicios)) { ?>
                    <tr>
                        <td><?php echo htmlspecialchars($row['codigo']); ?></td>
                        <td><?php echo htmlspecialchars($row['cliente']); ?></td>
                        <td><?php echo htmlspecialchars($row['fecha']); ?></td>
                        <td>
                            <span class="badge <?php echo ($row['estado'] == 'Pendiente') ? 'estado-cortado' : 'estado-pagado'; ?>">
                                <?php echo htmlspecialchars($row['estado']); ?>
                            </span>
                        </td>
                        <td>
                            <a href="agendar.php?id=<?php echo $row['cliente'] ?>& registro=<?php echo $row['id'] ?>" class="btn-action btn-edit" style="padding: 5px 10px;">Editar</a>
                            
							<br>
							<br>
							
							<form action="ingreso.php" method="post">
    <input name="cliente" type="hidden" value="<?php echo htmlspecialchars($row['codigo']); ?>">
    <input name="contratonumero" type="hidden" value="<?php echo htmlspecialchars($row['contrato']); ?>">
    <input name="estadousuario" type="hidden" value="incompleto">
    
    <button type="submit" class="btn-action btn-contrato" style="padding: 5px 10px; border: none; cursor: pointer;">
        Resolver
    </button>
</form>
                        </td>
