...
.menu-section a:hover{transform:translateX(6px);background:rgba(0,255,255,.08)}
    ...
    <main class="content">
      ...
      <!-- InstanceBeginEditable name="principal" -->
		<?php 
// 1. Recepción de fechas (default: mes actual)
$fecha_inicio = $_GET['fecha_inicio'] ?? date('Y-m-01');
$fecha_fin = $_GET['fecha_fin'] ?? date('Y-m-d');

// 2. Consulta datos cuenta y registros filtrados
$info_cuenta = ['institucion' => 'No disponible', 'saldo' => '0', 'responsable' => 'N/A', 'numero' => 'N/A'];
if (isset($_GET['codigo'])) {
    $codigo = mysqli_real_escape_string($con, $_GET['codigo']);
    $sql_info = "SELECT * FROM `cuentas` WHERE `numero` LIKE '$codigo' LIMIT 1";
    $res_info = mysqli_query($con, $sql_info);
    if($res_info && mysqli_num_rows($res_info) > 0) {
        $info_cuenta = mysqli_fetch_assoc($res_info);
    }
    
    // Consulta filtrada usando las fechas seleccionadas (incluye el día inicio y día fin)
    $sql2 = "SELECT * FROM `registro` WHERE `proveedor` LIKE '$codigo' 
...
<div class="cliente-wrapper">
    <div class="info-panel-wrapper">
        <div class="cliente-profile">
            <div class="avatar-big">CC</div>
            <h3><?php echo $info_cuenta['institucion']; ?></h3>
            <p>ID: <?php echo $info_cuenta['numero']; ?></p>
            <div class="saldo-destacado">Saldo: <?php echo number_format($info_cuenta['saldo'], 2); ?></div>
        </div>

        <div class="panel-dark">
            <div class="clientes-header-top">
                <h2 class="clientes-title">Filtrar Kardex</h2>
                <div class="clientes-inline">
                    <form method="GET" style="display:flex; gap:10px; align-items:center;">
                        <input type="hidden" name="codigo" value="<?php echo htmlspecialchars($_GET['codigo'] ?? ''); ?>">
                        <input type="date" name="fecha_inicio" value="<?php echo $fecha_inicio; ?>" class="clientes-input-small">
                        <input type="date" name="fecha_fin" value="<?php echo $fecha_fin; ?>" class="clientes-input-small">
                        <button type="submit" class="boton-azul">Buscar</button>
                    </form>

                    <?php if ($puesto_personal == "admin" OR $exportar == "si"): ?>
                        <button id="btnExportar" class="btn-action btn-edit">
                            <img src="../images/sistema/44.png" width="64" height="38" alt="CSV"/><br> Hoja de Calculo
                        </button>
                        <button id="btnPdf" class="btn-action btn-contrato">
                            <img src="../images/sistema/46.png" width="64" height="38" alt="PDF"/><br> Acrobat Reader
                        </button>
                    <?php endif; ?>
                </div>
            </div>
            <div class="cliente-info-grid">
                <div class="data-card"><div class="data-label">Rango</div><div class="data-value"><?php echo $fecha_inicio . ' al ' . $fecha_fin; ?></div></div>
                <div class="data-card"><div class="data-label">Sumatoria Periodo</div><div class="data-value" id="totalPeriodo">$0.00</div></div>
                <div class="data-card"><div class="data-label">Saldo Actual</div><div class="data-value">$<?php echo number_format($info_cuenta['saldo'], 2); ?></div></div>
            </div>
        </div>
    </div>

    <div class="panel-dark">
        <div class="clientes-header-top"><h2 class="clientes-title">KARDEX DE CUENTAS</h2></div>
        <div class="table-scroll">
            <table class="table-dark" id="tabla">
                <thead>
                    <tr>
                        <th>DOCUMENTO</th><th>RECIBO</th><th>FECHA</th><th>CONCEPTO</th>
                        <th>ACCIÓN</th><th>USUARIO</th><th>VALOR</th><th>SALDO</th><th>DETALLE</th>
                    </tr>
                </thead>
                <tbody>
                    <?php while($crow2 = mysqli_fetch_assoc($result2)): 
                        $valor = $crow2['saldo'];
                        if ($crow2['accion'] == "ingreso" || $crow2['accion'] == "pago") { $saldo += $valor; } else { $saldo -= $valor; }
                    ?>
                    <tr>
                        <td><?php echo $crow2['producto']; ?></td>
                        <td><?php echo $crow2['numerorecibo']; ?></td>
                        <td><?php echo $crow2['fecha']; ?></td>
                        <td><?php echo $crow2['hora']; ?></td>
                        <td><?php echo ucfirst($crow2['accion']); ?></td>
                        <td><?php echo $crow2['usuario']; ?></td>
                        <td class="valor-celda"><?php echo number_format($valor, 2); ?></td>
...
// Sumatoria dinámica
let total = 0;
document.querySelectorAll('.valor-celda').forEach(celda => {
    // Limpiamos formato de moneda antes de sumar
    total += parseFloat(celda.innerText.replace(/,/g, ''));
});
document.getElementById('totalPeriodo').innerText = '$' + total.toLocaleString(undefined, {minimumFractionDigits: 2});
...
document.getElementById('btnExportar').addEventListener('click', function() {
    ...
    let csv = "\ufeff" + Array.from(tabla.querySelectorAll('tr')).map(tr => 
        Array.from(tr.querySelectorAll('th, td')).map(cell => '"' + cell.innerText.replace(/"/g, '""') + '"').join(';')
    ).join('\n');
    let blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    let link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = 'Kardex_<?php echo $fecha_inicio; ?>_al_<?php echo $fecha_fin; ?>.csv';
    link.click();
});
</script>
