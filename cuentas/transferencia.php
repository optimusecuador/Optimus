...
.menu-section a:hover{transform:translateX(6px);background:rgba(0,255,255,.08)}
	...
	<?php
// Lógica de datos (se mantiene tu consulta)
$tabla = "cuentas";
$accion = $_GET['accion'] ?? '';
$sql = "SELECT * from `".$tabla."` order by numero DESC";
$result = mysqli_query($con, $sql);

// Consulta para verificar si hay cuentas
$sqlcontrol = "SELECT count(*) as total from cuentas";
$resultcontrol = mysqli_query($con, $sqlcontrol);
$dataControl = mysqli_fetch_assoc($resultcontrol);
?>

<div class="cliente-wrapper">
    <div class="panel-dark">
        <h2 class="clientes-title" style="text-align:center; margin-bottom: 20px;">TRANSFERENCIA DE CAJAS</h2>
        
        <form action="confirmacion_transferencia.php" method="post" name="form1" id="form1" class="clientes-form-panel">
            <div class="clientes-form-grid">
                
                <div class="clientes-field clientes-full">
                    <label>ORIGEN:</label>
                    <select name="producto" id="producto" class="clientes-input" required>
                        <?php while($crow = mysqli_fetch_assoc($result)) { ?>
                            <option value="<?php echo $crow['numero']; ?>"><?php echo htmlspecialchars($crow['institucion']); ?></option>
                        <?php } ?>
                    </select>
                </div>
                ...
                <div class="clientes-field clientes-full">
                    <label>DESTINO:</label>
                    <select name="destino" id="destino" class="clientes-input" required>
                        <?php 
                        // Rebobinamos el puntero del resultado o ejecutamos nueva consulta si es necesario
                        mysqli_data_seek($result, 0); 
                        while($crow7 = mysqli_fetch_assoc($result)) { ?>
                            <option value="<?php echo $crow7['numero']; ?>"><?php echo htmlspecialchars($crow7['institucion']); ?></option>
                        <?php } ?>
                    </select>
                </div>
                ...
                <div class="clientes-field">
                    ...
                    <input type="text" name="cantidad" required id="cantidad" class="clientes-input" placeholder="0.00">
                </div>

                <div class="clientes-field">
                    <label>CONCEPTO:</label>
                    <input type="text" name="concepto" required id="concepto" class="clientes-input">
                </div>
            </div>

            <input type="hidden" name="accion" value="<?php echo htmlspecialchars($accion); ?>">
            <input type="hidden" name="accion2" value="<?php echo htmlspecialchars($accion); ?>">
            <input type="hidden" name="form_id" value="<?php echo htmlspecialchars($codigo ?? ''); ?>">

            <div style="margin-top: 20px; text-align: right;">
                <button type="submit" name="submit2" class="btn-action btn-contrato">GUARDAR TRANSFERENCIA</button>
            </div>
        </form>
    </div>
</div>
...
<?php
// Verificación
if ($dataControl['total'] == 0) {
    echo "<script>alert('No tiene Cuentas Configuradas -----> Cuentas Bancarias/Nueva Cuenta');</script>";
}
?>
