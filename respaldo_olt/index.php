...
$sqlolt = "SELECT * FROM olt_conexion LIMIT 1";
...
if(isset($_POST['respaldar']))
{
    ...
    $conn = ssh2_connect($host_olt,$port);

    if(!$conn)
    {
        echo "<script>
        alert('No conecta con la OLT');
        </script>";
    }
    elseif(!ssh2_auth_password($conn,$u_olt,$p_olt))
    {
        echo "<script>
    ...
    else
    {
        ...
        file_put_contents(
            $carpeta_respaldo.'/'.$archivo,
            $config
        );

        mysqli_query(
            $con,
            "INSERT INTO respaldo_olt(
                archivo,
                fecha
            ) VALUES (
                '$archivo',
        ...
        if($rowTotal['total'] > 30)
        {
            ...
            while($viejo=mysqli_fetch_assoc($sqlViejos))
            {
                ...
                $ruta = $carpeta_respaldo.'/'.$viejo['archivo'];
...
<a
class="boton-azul"
href="../respaldo_olt/<?php echo $row['archivo']; ?>"
target="_blank">
Descargar
</a>
...
name="restaurar"
class="boton-azul"
onclick="return confirm('¿Desea restaurar este respaldo?');">
Restaurar
</button>
