...
.menu-section a:hover{transform:translateX(6px);background:rgba(0,255,255,.08)}
	...
	<div>
    <h2 style='color:white'>TRACCAR</h2>
</div>

<?php 
// Inicialización de variables para Traccar
$traccar_url = "Vacio";
$traccar_user = "Vacio";
$traccar_pass = "Vacio";
?>
...
<!-- Column -->
<?php 
// Buscamos el registro en la tabla traccar
$sql_tc = "SELECT ip, api, contrasena FROM `traccar` LIMIT 1";
$result_tc = mysqli_query($con, $sql_tc);

// Si existe el registro, asignamos los valores
if ($result_tc && mysqli_num_rows($result_tc) > 0) {
    $row_tc = mysqli_fetch_assoc($result_tc);
    $traccar_url  = $row_tc['ip'];
    $traccar_user = $row_tc['api'];
    $traccar_pass = $row_tc['contrasena'];
}
?>    

<div class="panel-dark">
    <div class="cliente-info-title">Configuración Traccar</div>
    
    <form action="guardar_traccar.php" method="post" name="form_traccar" id="form_traccar">
        <table width="90%" class="table-dark">
            <tbody>
                <tr>
                    <td><strong>IP / URL de acceso:</strong></td>
                    <td><input class="clientes-input" name="ip" type="text" id="ip" value="<?php echo htmlspecialchars($traccar_url); ?>"></td>
                    <td><strong>Usuario:</strong></td>
                    <td><input class="clientes-input" name="api" type="text" id="api" value="<?php echo htmlspecialchars($traccar_user); ?>"></td>
                </tr>
                <tr>
                    <td><strong>Contraseña:</strong></td>
                    <td colspan="3"><input class="clientes-input" name="contrasena" type="password" id="contrasena" value="<?php echo htmlspecialchars($traccar_pass); ?>"></td>
                </tr>
                <tr>
                    <td colspan="4" align="center"><br>
                        <span class="buttons">
                            <?php 
                            $accion = ($traccar_url == "Vacio") ? "nuevo" : "editar";
                            ?>
                            <input name="iporiginal" type="hidden" id="iporiginal" value="<?php echo htmlspecialchars($traccar_url); ?>">
